...
function createLoader()
	...
	local tfLoader = display.newText( "Loading...", 0, 0, nil, 40 )
...
function addButton(title, tx, ty, fname, onRelease, tfSize, tfColor, ico)
	...
	if (tfSize) then
		item_mc.tf = display.newText( title, 0, 0, fontMain, tfSize )
		if (tfColor) then
...
local function loadSounds()
	...
	greenSounds:add_sound( "musicBossFight" )
	greenSounds:add_sound( "musicGameLevel" )
	...
	local function mainHandler( e )
		if #loading_steps > 0 then
			...
			if screenLoader then
				...
				screenLoader.tf.text = "Loading: " .. ': ' .. loading_p .. '%'
			end
			return
		end
